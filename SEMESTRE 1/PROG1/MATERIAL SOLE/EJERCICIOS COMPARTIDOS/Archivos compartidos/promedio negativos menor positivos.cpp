...
real nro                                                        // Dato (a leer quizás varias veces)
    ...
    ,menorPos = -1.0;                                           // Menor de los positivos (inicializado en un valor incorrecto como resultado)
...
iterar
    ...
    si(nro > 0) entonces                                        // Si es positivo
        si(nro < menorPos O menorPos ES (-1.0)) entonces        // Si es el primer positivo o es menor que el que está
            menorPos = nro;
            finSi
