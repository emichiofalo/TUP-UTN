...
export async function getInstrumentos() {
  const url = '/instrumentos';
  const instrumentos = await getData<Instrumento[]>(url);
  return instrumentos;
}

export async function getInstrumentoById(id: number) {
  const url = `/instrumento/${id}`;
  const instrumento = await getData<Instrumento>(url);
  return instrumento;
}

//si el metodo es post, data no tiene que tener id
//si el metodo es put, data tiene que tener id 
async function postPutData<T>(path: string, method: string, data: T): Promise<T> {
  const response = await fetch(`http://localhost:8080${path}`, {
